...
class deform_network(nn.Module):
    def __init__(self, D=8, W=256, min_embeddings=30, max_embeddings=150, num_frames=300, num_cam=None, args=None) -> None:
        ...
        pos_fourier_dim = 3 + 2*10*3 # 3D 坐标 + 10 个频率的 sin/cos 编码
        self.mlp_f = self._make_mlp(args.temporal_embedding_dim + pos_fourier_dim, W, D, residual=True)
    ...
    def forward(
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            ...
            hidden_f = self.mlp_f(torch.cat([t_emb, pts_fourier], dim=-1))
