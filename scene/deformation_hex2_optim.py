...
def fourier_encode_v2(x: torch.Tensor, freq_buf: torch.Tensor) -> torch.Tensor:
    """
    优化版傅里叶编码:
    1. 避免创建完整(N,C,F)中间张量
    2. 并行计算sin/cos
    """
    # 直接计算展开维度的乘积
    expanded = x[..., None] * freq_buf  # (N, C, F)
    
    # 并行计算sin和cos
    sin_vals = torch.sin(expanded)
    cos_vals = torch.cos(expanded)
    
    # 拼接所有结果
    return torch.cat([
        x, 
        sin_vals.flatten(start_dim=-2), 
        cos_vals.flatten(start_dim=-2)
    ], dim=-1)
...
class deform_network(nn.Module):
        ...
        class MultiHeadPredictor(nn.Module):
            def __init__(self, in_dim, hidden, depth, out_dim, residual=False):
                ...
                self.head = nn.Sequential(
                    nn.ReLU(), nn.Linear(hidden, hidden), nn.ReLU(), nn.Linear(hidden, total_dim)
                )
                if residual:
                    self.shared.apply(kaiming_init_weights)
    ...
    def forward(
        ...
        # fourier 编码
        pts_fourier = fourier_encode_v2(pts, self.pos_freq)  # (N, 3 + 2*10)
        scales_fourier = fourier_encode_v2(scales, self.pos_freq)  # (N, 3 + 2*10)
        rotations_fourier = fourier_encode_v2(rotations, self.rot_scale_freq)  # (N, 4 + 2*2)
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            grid_feat = self.grid(pts, time).float()
            dx, ds, dr, do, dc = self.coarse_predictor(grid_feat)
            dc = dc.view(-1, 16, 3)  # 重新整形RGB系数
            ...
            # -------------- fine deform ------------------------
            # ---- Fine MLP ----
            dx_f, ds_f, dr_f, do_f, dc_f = self.fine_predictor(torch.cat([t_emb, gaussian_emb], dim=-1))  # (N, 3), (N, 3), (N, 4), (N, 1), (N, 16*3)
            pts_f = pts_c + dx_f * coef_main
            scl_f = scl_c + (
                ds_f * coef_main * coef_s
            ...
            opa_f = opa_c + (
                do_f * coef_main * coef_o
                if not getattr(self.args, "no_do", False)
                else 0.0
            )
            sh_f = sh_c + (
                dc_f.view(-1, 16, 3) * coef_main * coef_c
                if not getattr(self.args, "no_dc", False)
                else 0.0
            )
